'use client'

import { useState } from 'react'
import Link from 'next/link'
import { usePathname } from 'next/navigation'
import {
  LayoutDashboard, Package, Truck, MapPin, QrCode, ScanLine,
  BarChart3, Settings, Bell, Menu, X, LogOut, Warehouse,
  ClipboardList, CheckSquare
} from 'lucide-react'
import { cn } from '@/lib/utils'
import Avatar from '@/components/ui/Avatar'

const NAV_ITEMS = [
  { href: '/logistics',          icon: LayoutDashboard, label: 'Dashboard' },
  { href: '/logistics/incoming', icon: Package,         label: 'Qabul qilish' },
  { href: '/logistics/sorting',  icon: ClipboardList,   label: 'Saralash' },
  { href: '/logistics/dispatch', icon: Truck,           label: 'Jo\'natish' },
  { href: '/logistics/tracking', icon: MapPin,          label: 'Kuzatish' },
  { href: '/logistics/scan',     icon: ScanLine,        label: 'Skanerlash' },
  { href: '/logistics/reports',  icon: BarChart3,       label: 'Hisobotlar' },
]

export default function LogisticsLayout({ children }: { children: React.ReactNode }) {
  const pathname = usePathname()
  const [sidebarOpen, setSidebarOpen] = useState(false)

  return (
    <div className="min-h-screen bg-surface-50 dark:bg-surface-950 flex">
      <aside className={cn(
        'fixed inset-y-0 left-0 z-50 w-60 bg-surface-900 text-white flex flex-col transition-transform duration-300',
        'lg:translate-x-0',
        sidebarOpen ? 'translate-x-0' : '-translate-x-full'
      )}>
        <div className="flex items-center gap-3 px-4 h-16 border-b border-surface-800 shrink-0">
          <div className="h-8 w-8 bg-emerald-600 rounded-xl flex items-center justify-center">
            <Warehouse className="h-4 w-4 text-white" />
          </div>
          <div>
            <p className="text-sm font-bold">Market.uz</p>
            <p className="text-[10px] text-surface-400">Logistika tizimi</p>
          </div>
          <button onClick={() => setSidebarOpen(false)} className="ml-auto lg:hidden text-surface-400">
            <X className="h-5 w-5" />
          </button>
        </div>

        <div className="px-3 py-3 border-b border-surface-800 shrink-0">
          <div className="flex items-center gap-2.5 p-2.5 bg-surface-800 rounded-xl">
            <Avatar name="Logist" size="sm" />
            <div>
              <p className="text-sm font-semibold">Warehouse #1</p>
              <p className="text-[10px] text-emerald-400">Toshkent, Yunusobod</p>
            </div>
          </div>
        </div>

        <nav className="flex-1 overflow-y-auto p-3 space-y-0.5">
          {NAV_ITEMS.map(item => (
            <Link
              key={item.href}
              href={item.href}
              onClick={() => setSidebarOpen(false)}
              className={cn(
                'flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all',
                pathname === item.href
                  ? 'bg-emerald-600 text-white'
                  : 'text-surface-300 hover:bg-surface-800 hover:text-white'
              )}
            >
              <item.icon className="h-4 w-4 shrink-0" />
              {item.label}
            </Link>
          ))}
        </nav>

        <div className="p-3 border-t border-surface-800 shrink-0">
          <Link href="/" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-surface-400 hover:bg-surface-800 hover:text-white transition-all">
            <LogOut className="h-4 w-4" />
            Chiqish
          </Link>
        </div>
      </aside>

      {sidebarOpen && (
        <div className="fixed inset-0 z-40 bg-black/50 lg:hidden" onClick={() => setSidebarOpen(false)} />
      )}

      <div className="flex-1 lg:ml-60 flex flex-col min-h-screen">
        <header className="sticky top-0 z-30 h-16 bg-white dark:bg-surface-900 border-b border-surface-100 dark:border-surface-800 flex items-center gap-4 px-4">
          <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 rounded-xl hover:bg-surface-100 dark:hover:bg-surface-800">
            <Menu className="h-5 w-5 text-surface-600 dark:text-surface-400" />
          </button>
          <h1 className="text-base font-bold text-surface-900 dark:text-white hidden sm:block">
            {NAV_ITEMS.find(i => i.href === pathname)?.label || 'Logistika'}
          </h1>
          <div className="ml-auto flex items-center gap-2">
            <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl">
              <span className="h-2 w-2 bg-emerald-500 rounded-full animate-pulse-soft" />
              <span className="text-xs font-semibold text-emerald-700 dark:text-emerald-400">Ombor faol</span>
            </div>
            <button className="relative p-2 rounded-xl hover:bg-surface-100 dark:hover:bg-surface-800">
              <Bell className="h-5 w-5 text-surface-600 dark:text-surface-400" />
              <span className="absolute top-1.5 right-1.5 h-2 w-2 bg-red-500 rounded-full" />
            </button>
            <Avatar name="Logist" size="sm" />
          </div>
        </header>
        <main className="flex-1 p-4 md:p-6">{children}</main>
      </div>
    </div>
  )
}
