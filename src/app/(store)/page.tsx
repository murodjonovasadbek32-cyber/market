...
function HeroBanner() {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl h-[420px] md:h-[500px]" onMouseEnter={() => setAutoPlay(false)} onMouseLeave={() => setAutoPlay(true)}>
        ...
        >
          ...
          <div className="relative z-10 h-full flex items-center px-8 md:px-12">
            <div className="max-w-lg">
              ...
              <p className={cn('text-lg md:text-xl font-semibold mb-2', slide.accent)}>
                {slide.subtitle}
              </p>
              ...
              <Link
                href={slide.href}
                className="inline-flex items-center gap-2 px-6 py-3 bg-white text-surface-900 font-semibold rounded-2xl hover:bg-surface-100 transition-colors text-sm"
              >
                {slide.cta}
      ...
      <button
        onClick={() => { setAutoPlay(false); setCurrent((c) => (c - 1 + heroData.length) % heroData.length) }}
        className="absolute left-3 top-1/2 -translate-y-1/2 z-20 h-10 w-10 bg-black/30 hover:bg-black/50 backdrop-blur-sm text-white rounded-xl flex items-center justify-center transition-all"
      >
        <ChevronLeft className="h-5 w-5" />
      </button>
      <button
        onClick={() => { setAutoPlay(false); setCurrent((c) => (c + 1) % heroData.length) }}
        className="absolute right-3 top-1/2 -translate-y-1/2 z-20 h-10 w-10 bg-black/30 hover:bg-black/50 backdrop-blur-sm text-white rounded-xl flex items-center justify-center transition-all"
      >
    ...
    <div className="flex items-center gap-1.5">
      {[time.h, time.m, time.s].map((v, i) => (
        <span key={i} className="flex items-center gap-1">
          <span className="min-w-[2rem] h-8 bg-surface-900 dark:bg-surface-950 text-white text-sm font-bold rounded-lg flex items-center justify-center tabular-nums">
  ...
  return (
    <div className="bg-surface-50 dark:bg-surface-950 min-h-screen">
        ...
        <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-12 gap-3">
          {categories.map((cat) => (
            <Link
              ...
              className="flex flex-col items-center gap-2 p-3 bg-white dark:bg-surface-900 rounded-2xl border border-surface-100 dark:border-surface-800 hover:border-purple-200 dark:hover:border-purple-800 hover:shadow-card transition-all duration-200 group"
            >
                  ...
                  <div className="p-3">
                    ...
                    <div className="mt-2">
                      <div className="flex items-center justify-between text-[10px] text-surface-400 mb-1">
                        <span>Sotildi: {Math.floor(Math.random() * 80 + 20)}%</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <Link
              key={banner.title}
              href={banner.href}
              className={cn('relative rounded-2xl overflow-hidden h-40 bg-gradient-to-r group', banner.bg)}
            >
              ...
              <div className="absolute inset-0 p-5 flex flex-col justify-between">
                <span className="self-start px-2 py-1 bg-white/20 backdrop-blur-sm text-white text-xs font-bold rounded-lg">
                  {banner.discount}
                </span>
                <div>
      ...
      <section className="max-w-[1400px] mx-auto px-4 pb-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {products.filter(p => p.isBestseller).concat(products.slice(0, 3)).slice(0, 5).map((product) => (
            <ProductCard key={product.id} product={product} />
          ))}
        </div>
      </section>
        ...
        <div className="bg-surface-900 dark:bg-surface-950 rounded-3xl p-8 md:p-12 flex flex-col md:flex-row items-center gap-8">
          <div className="flex-1">
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 px-6 py-3 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-2xl transition-colors text-sm"
            >
              Hoziroq boshlang
              <ArrowRight className="h-4 w-4" />
            </Link>
